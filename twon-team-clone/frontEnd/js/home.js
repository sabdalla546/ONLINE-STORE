    ...
    (async function getAllProduct(){
        ...
        for(let i=0 ;i < res.data.data.products.length; i++){
             ...
             <div class="productDetails">
                 ...
                 <button class="addToCart" product-id=${res.data.data.products[i]._id}>Add To Cart</button>
             </div>
         </div>
             `
        }
        document.getElementById('newProductsApi').innerHTML= content;
        
     })();   

     document.getElementById('newProductsApi').addEventListener('click',async e=>{
